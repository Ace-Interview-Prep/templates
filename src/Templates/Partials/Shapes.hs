...
-- Example: circle (twSize' 10) (only (noTransition (solidColor aceAccent))) (only White)
circle :: DomBuilder t m => TWSizeOrFraction -> WhenTW (WithTransition GradientColor) -> WhenTW Color -> m ()
...
circle' :: DomBuilder t m => Radius -> WhenTW (WithTransition GradientColor) -> WhenTW Color -> m ()
circle' rad bgCol txtCol = circle (twSize' (rad * 2)) bgCol txtCol

-- | Circle with dynamic color
circleDynColor :: Template t m => TWSizeOrFraction -> Dynamic t GradientColor -> Dynamic t Color -> m ()
